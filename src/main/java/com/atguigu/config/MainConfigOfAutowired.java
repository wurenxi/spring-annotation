 ...
 *      1.5 @Primary：让Spring进行自动装配时，默认使用首选的bean
 *              也可以继续使用@Qualifier指定需要装配的bean的名字
 *      BookService{
 *          @Autowired
 *          BookDao bookDao;
 *      }
 * 2）Spring还支持使用@Resource(JST250)和@Inject(JSR330)[java规范的注解]
 *      @Resource：
 *          可以和@Autowired一样实现自动装配功能；默认是按照组件名称进行装配的
 *          没有能支持@Primary功能，没有支持@Autowired(required=false)
 *      @Inject：
 *          需要导入javax.inject的包，和@Autowired的功能一样。没有required=false的功能
 *  @Resource：Spring定义的；@Resource、@Inject都是java规范
 *
 * AutowiredAnnotationBeanPostProcessor：解析完成自动装配功能
 *
 * 3）@Autowired：构造器、参数、方法、属性。都是从容器中获取参数组件的值
 *      3.1 [标注在方法位置]：@Bean+方法参数；参数从容器中获取；默认不写@Autowired，效果一样的，都能自动装配
 *      3.2 [标在构造器上]；如果组件只有一个有参构造器，这个有参构造器的@Autowired可以省略，参数位置的组件还是可以自动从容器中获取
 *      3.3 放在参数位置
 *
 * 4）自定义组件想要使用Spring容器底层一些组件(ApplicationContext,BeanFactory,xxx)
